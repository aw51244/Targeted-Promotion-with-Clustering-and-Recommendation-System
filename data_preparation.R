...
# join the transaction table with product information
...
# exclude customers that do not fit for promotion
...
max(discount_2017$total_discount) 

fwrite(cust_2017,"cust_2017.csv")
### navigate to Python, to do the cluster and recommender....
...
# compute most popular brand in each subcategory with sub_category name
whole_table_2017[sub_category_desc=="POLVO CONGELADO EMB",
                 .(spend_POLVO=sum(tran_prod_paid_amt)),
                 by = .(brand_desc)][order(-spend_POLVO)]

# compute most popular brand in each subcategory with sub_category id
